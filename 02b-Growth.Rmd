...
**Using `Stan` with a normal likelihood.**

We used the following growth model with a normal distribution to estimate individual growth potential:

\begin{equation} 
  log(DBH_{y=today,s,i}  - DBH_{y=y0,s,i}) \sim \\
  \mathcal{N} (log(\sum _{y=y_0} ^{y=today} \theta_{1,s,i}.exp(-\frac12.[\frac{log(\frac{DBH_{y,s,i}}{100.\theta_{2,s}})}{\theta_{3,s}}]^2)), \sigma_1) \\ 
...
```{r thetaib, fig.height=12, eval=F}
