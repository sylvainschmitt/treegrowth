...
(g.igv <- summarise(group_by(growth, species), cvlog_gmaxi  = sqrt(exp(var(log(gmax)))-1), median_gmaxi  = median(gmax),
  ...
  ggtitle("X.", expression(beta==-0.07~p==0.018~R^2==0.015))) 
```
...
(g.phylo <- fortify(phylo) %>% 
  ...
  ggtitle("C.", expression(Pagel~lambda==0.72~p<0.001))) 
```
...
```{r figall2, fig.width=10, fig.height=5}
gridExtra::grid.arrange(
  grobs = list(g.phylo,  g.nci),
