...
ggplot(growth, aes(NCI, gmax, col = species, group = NA)) +
  ...
  geom_smooth(method = "lm", aes(group = NA), se = T, col = "red") +
...
```{r rlmigv}
MASS::rlm(cvlog_gmaxi ~ poly(median_gmaxi, 2), method="MM", summarise(group_by(growth, species), cvlog_gmaxi  = sqrt(exp(var(log(gmax)))-1), median_gmaxi  = median(gmax))) %>% 
  sjPlot::tab_model(show.icc = F) 
```
...
fortify(phylo) %>% 
  ...
  viridis::scale_color_viridis(expression(median(Gmax[i])~cm.year^-1), trans = "log") + xlim(NA, 200)
```
...
g.igv <- summarise(group_by(growth, species), cvlog_gmaxi  = sqrt(exp(var(log(gmax)))-1), median_gmaxi  = median(gmax)) %>% 
  ...
  xlab(expression(Median(Gmax[i])~cm.year^-1)) + ylab(expression(CV[log](Gmax[i])~cm.year^-1)) +
  ggtitle("C.", expression(beta==-0.56~p==0.018~R^2==0.146))
