...
g.phylo <- fortify(phylo) %>% 
  ...
  ggtitle("A.", expression(Pagel~lambda==0.72~p<0.001))
g.nci <- ggplot(growth, aes(NCI, gmax, col = species, group = NA)) +
  ...
  ggtitle("B.", expression(beta==-0.89~p<0.001~R^2==0.109))
g.igv <- summarise(group_by(growth, species), cvlog_gmaxi  = sqrt(exp(var(log(gmax)))-1), median_gmaxi  = median(gmax)) %>% 
  ggplot(aes(median_gmaxi, cvlog_gmaxi)) +
  geom_point() +
  geom_smooth(method = "glm", formula= y ~ poly(x,2)) +
  xlab(expression(Median(Gmax[i])~cm.year^-1)) + ylab(expression(CV[log](Gmax[i])~cm.year^-1)) +
  ggtitle("C.", expression(gamma==-1.18~p<0.001~R^2==0.146))
gridExtra::grid.arrange(
  grobs = list(g.phylo,  g.nci, g.igv),
