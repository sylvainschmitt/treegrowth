...
p4d <- phylo4d(phylo,
               data.frame(species = gsub("_", " ", phylo$tip.label)) %>% 
                 left_join(group_by(growth, species) %>% summarise(igv  = var(gmax), gmax  = median(gmax))) %>% 
...
```{r lmigv}
lm(igv ~ poly(gmax, 2), summarise(group_by(growth, species), igv  = var(gmax), gmax  = median(gmax))) %>% 
  sjPlot::tab_model(show.icc = F)
```
...
summarise(group_by(growth, species), igv  = var(gmax, na.rm = T), gmax  = median(gmax)) %>% 
  ggplot(aes(gmax, igv)) +
  geom_point() +
  geom_smooth(method = "glm", formula= y ~ poly(x,2)) +
  xlab(expression(median(Gmax[i]))) + ylab(expression(variance(Gmax[i]))) 
```
...
fortify(phylo) %>% 
  ...
  viridis::scale_color_viridis(expression(median(Gmax[i])), trans = "log")
```
...
fortify(phylo) %>% 
  mutate(species = gsub("_", " ", label)) %>%
  mutate(label = species) %>% 
  left_join(group_by(growth, species) %>% summarise(igv = var(gmax, na.rm = T))) %>% 
  ggtree(aes(color = igv), layout="circular") + 
  geom_tiplab2(size = 2) +
  theme_tree(legend.position='right', legend.text = element_text(face = "italic")) +
  scale_alpha_manual("taxon", values = c(0.2, 1)) +
  scale_size_manual("taxon", values = c(1, 2)) +
  viridis::scale_color_viridis(expression(variance(Gmax[i])), trans = "log")
```
