...
```{r, eval=F}
read_tsv("data/growthfull_binded.tsv") %>% 
  # ggplot(aes(species, Mean)) + geom_boxplot() + coord_flip() + scale_y_sqrt()
  group_by(species) %>% 
  mutate(hi = Mean/quantile(Mean, 0.95, na.rm = T)) %>% 
  # ggplot(aes(species, hi)) + geom_boxplot() + coord_flip() + scale_y_sqrt()
  # ggplot(aes(hi, gmax)) + geom_point() + scale_y_log10() + geom_smooth(method = "lm")
  lme4::lmer(log(gmax) ~ log(NCI) + hi + (1  | Family/Genus/species), .) %>% 
