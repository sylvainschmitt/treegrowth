...
```{r growthguyafortrace, eval=F}
vroom::vroom('save/growthguyafor/sigma.tsv', col_names = c("iteration", "parameter", "value")) %>% 
  ggplot(aes(iteration, value)) +
  geom_line() +
  facet_wrap(~ parameter, scales = "free_y", nrow = 3)
```

```{r growthguyaforgmax, eval=F}
vroom::vroom('save/growthguyafor/gmax.tsv') %>% 
  filter(ind %in% 1:100) %>% 
  group_by(ind) %>% 
  summarise(ll = quantile(value, 0.05), l = quantile(value, 0.25), 
            m = quantile(value, 0.5), 
            h = quantile(value, 0.75), hh = quantile(value, 0.95)) %>% 
  ggplot(aes(x = ind, xend = ind)) +
...
```{r growthguyafordoptdmax, eval=F}
vroom::vroom('save/growthguyafor/dopt.tsv', col_names = c("iteration", "parameter", "value")) %>% 
  group_by(parameter) %>% 
  summarise(value = median(value)) %>% 
  separate(parameter, c("dopt", "s", "sp")) %>% 
  mutate(sp = as.numeric(sp)) %>% 
  left_join(vroom::vroom("save/mdataguyafor.tsv") %>% 
              group_by(sp, Family, Genus, Species) %>% 
              summarise(dmax = max(DBH))) %>% 
  ggplot(aes(y = sp, x = value/dmax*100, col = value/dmax>1)) +
  geom_point() +xlab(expression(frac(Dopt,Dmax))) + ylab("Species") + 
...
```{r parsanc, eval=F}
draws <- vroom::vroom('save/growthguyafor/gmax.tsv') %>% 
  group_by(ind) %>% 
...
```{r gmaxphyloc, eval=F, fig.cap="Distribution of species growth potential (Gmax, cm/yr) in the phylogeny."}
g <- fortify(phylo) %>% 
...
ggsave(g, filename = "save/figs/phylogeny.png", dpi = 500, width = 25, height = 25, units = "cm", bg = 'white')
include_graphics("save/figs/phylogeny.png")
```
