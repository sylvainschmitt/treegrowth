...
The result is summarised in Table ...
...
trees %>% 
  ...
  vroom::vroom_write("save/trees_reduced.tsv")
vroom::vroom("save/trees_reduced.tsv") %>% 
  group_by(idTree) %>% 
  filter(CensusYear == max(CensusYear)) %>% 
  vroom::vroom_write("save/eval_reduced.tsv")
vroom::vroom("save/trees_reduced.tsv") %>% 
  group_by(idTree) %>% 
  filter(CensusYear != max(CensusYear)) %>% 
...
ggplot(vroom::vroom("save/mdata_reduced.tsv"),
       aes(Year, DBH, col = as.factor(ind))) +
  geom_point() +
  geom_point(data = vroom::vroom("save/eval_reduced.tsv"), col = "red", size = 2) +
  geom_smooth(se = F) +
...
```{r mmstanfit, eval=F}
mdata <- vroom::vroom("save/mdata_reduced.tsv")
eval <- vroom::vroom("save/eval_reduced.tsv")
...
save(fit_mm, file = "save/mm.Rdata")
```
...
save(fit_amani, file = "save/amani.Rdata") 
```

```{r amanisampling}
load("save/amani.Rdata")  
```
...
pars <- as.data.frame(fit_gompertzsum, pars = c('gmax', "dopt", "ks")) %>% 
  ...
  separate(parameter, c("parameter", "ind"), convert = T) %>% 
  reshape2::dcast(ind ~ parameter)
left_join(vroom::vroom("save/mdata_reduced.tsv"), pars) %>% 
  rowwise() %>% 
