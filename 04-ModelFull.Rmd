...
In this chapter, I fitted the selected model with full data.
...
#   vroom::vroom_write("save/rhatfull.tsv")
vroom::vroom("save/rhatfull.tsv") %>% 
  separate(parameter, c("parameter", "indsp")) %>% 
  filter(parameter == "gmax") %>% 
  select(rhat) %>% 
  unlist() %>% 
  mcmc_rhat() +
  ggtitle("8569 Gmax_i", "608 with rhat > 1 (7%)")
```

```{r growthfulltrace}
# g <- mcmc_trace(as.array(fit_full, pars = c("gmax[1]", "dopt[1]", "ks[1]", "sigma", "sigmaD", "sigmaK")))
# ggsave(plot = g, filename = "save/figs/growthfulltrace.png", bg = "white")
include_graphics("save/figs/growthfulltrace.png")
```

```{r growthfullpairs}
# g <- mcmc_pairs(as.array(fit_full, pars = c("gmax[1]", "dopt[1]", "ks[1]", "sigma", "sigmaD", "sigmaK")))
# ggsave(plot = g, filename = "save/figs/growthfullpairs.png", bg = "white")
include_graphics("save/figs/growthfullpairs.png")
```

```{r growthfullgmax}
# g <- mcmc_intervals(as.array(fit_full, pars = paste0("gmax[", 1:100, "]")))
# ggsave(plot = g, filename = "save/figs/growthfullgmax.png", bg = "white")
include_graphics("save/figs/growthfullgmax.png")
```
...
However, 11 species showed a $Dopt_s$ above the observed 95th quantile of their observed diameters.
...
# ggsave(plot = g, filename = "save/figs/growthfulldoptdmax.png", bg = "white")
include_graphics("save/figs/growthfulldoptdmax.png")
```

```{r growthfulldoptdmaxsp}
# sps <- mcmc_intervals_data(as.array(fit_full, pars = "dopt_s")) %>%
#   separate(parameter, c("parameter", "X1", "sp"), convert = T) %>%
#   left_join(mdata %>%
#               group_by(sp) %>%
#               summarise(dmax = max(DBH))) %>%
...
#   select(sp) %>%
#   unlist()
# g <- dplyr::select(vroom::vroom("save/mdatafull.tsv"), sp, ind) %>%
#   unique() %>%
#   filter(sp %in% sps) %>%
#   group_by(sp) %>%
#   unique() %>%
...
# ggsave(plot = g, filename = "save/figs/growthfulldoptdmaxsp.png", bg = "white")
include_graphics("save/figs/growthfulldoptdmaxsp.png")
```