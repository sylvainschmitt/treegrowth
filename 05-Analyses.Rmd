...
```{r gmaxtwisp, fig.cap="Relation between Neighbourhood crowding index (NCI) and individual growth potential (Gmax, cm/yr).", eval=F}
growth %>% 
  group_by(species) %>% 
  mutate(twi = exp(mean(log(twi+1))), gmax = exp(mean(log(gmax)))) %>% 
  ggplot(aes(twi, gmax)) +
  geom_point() +
  geom_smooth(method = "lm", aes(group = NA)) +
  scale_color_discrete(guide = "none") +
