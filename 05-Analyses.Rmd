...
```{r igvphylo, fig.cap="Distribution of intraspecifig growth variability (IGV, cm/yr) in the phylogeny."}
fortify(phylo) %>% 
  mutate(species = gsub("_", " ", label)) %>%
  mutate(label = species) %>% 
...
```{r igvcorrelog, eval=T}
# crlg <- phyloCorrelogram(p4d, trait = "logigv", ci.bs	= 100)
# png(file = "save/figs/igvcorrelog.png", width = 1000, height = 1000, units = "px")
# plot(crlg, main="log(IGV) phylogenetic correlogram")
# dev.off()
include_graphics("save/figs/igvcorrelog.png")
```
