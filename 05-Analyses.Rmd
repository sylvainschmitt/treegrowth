...
vroom::vroom("save/mdatahalf.tsv") %>% 
  filter(species == "Swartzia polyphylla") %>% 
  filter(DBH < 10) %>% 
  ggplot(aes(CensusYear, group = as.factor(ind))) +
  geom_point(aes(y = CircCorr)) +
  geom_line(aes(y = CircCorr, col = "Corrected")) +
  geom_point(aes(y = Circ)) +
  geom_line(aes(y = Circ, col = "Raw")) +
  facet_wrap(~idTree)
```
...
Most of the variation in growth potential is individual ($\sigma_i = 0.40$), then explained by genus ($\sigma_g = 0.09$)
before species ($\sigma_s=0.03$) and relatively little by family ($\sigma_f=0.2$). 
The taxonomic structure explains more than a quarter of the observed variation in individual growth potential ($R^2_c=0.28$).
Finally, the neighbourhood ($NCI$) has a marked negative significant effect ($\beta=-0.45, p<0.001$, Fig. \@ref(fig:gmaxnci)) which explains 3% of the observed variation ($R^2_m=0.033$).
...
```{r r2, eval=F }
m <- lme4::lmer(log(gmax) ~ log(NCIs) + log(NCIis) + log(TWIs+1) + log(TWIis+1) + (1  | Family/Genus/species), growth)
                na.omit(dplyr::select(growth, Family, Genus, species, NCIs, NCIis, TWIs, TWIis, gmax)) %>% mutate(TWIis = ifelse(TWIis <= 0, 10^-6, TWIis)))
...
Species growth potential in the phylogeny (Fig. \@ref(fig:gmaxphylo)) is significantly structured (Tab. \@ref(tab:gmaxphylosignal)),
with a short distance significant positive association and a long distant significant negative association (Fig. \@ref(fig:gmaxcorrelog)).
Local indicator of phylogenetic association highlight the conservation of species growth potential at the genus level (Fig. \@ref(fig:lipa)),
