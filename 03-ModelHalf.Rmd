...
The model correctly converged ($\hat R < 1.05$) for all except one $Gmax_i$ (still below $\hat R < 1.1$).
The parameters doesn't show correlations.
$Gmax_i$ posteriors have logical uncertainty but are varying widely among individuals and are not constrained by the superior limit.
$Dopt_s$ are all strictly below $Dmax$.
...
#   ggplot(aes(sp, m/dmax, col = m/dmax > 1)) +
#   geom_point() +
#   coord_flip() +
#   scale_y_sqrt() +
